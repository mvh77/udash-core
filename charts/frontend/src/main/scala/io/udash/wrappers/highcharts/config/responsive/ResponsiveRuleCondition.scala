...
object ResponsiveRuleCondition {
  ...
  /**
    ...
    */
  def apply(callback: Option[(api.Chart) => Boolean] = None,
            maxHeight: Option[Double] = None,
            maxWidth: Option[Double] = None,
            minHeight: Option[Double] = None,
            minWidth: Option[Double] = None): ResponsiveRuleCondition = {
    val callbackOuter = callback.map(ThisFunction.fromFunction1[api.Chart, Boolean]).orUndefined
    val maxHeightOuter = maxHeight.orUndefined
    val maxWidthOuter = maxWidth.orUndefined
    val minHeightOuter = minHeight.orUndefined
    val minWidthOuter = minWidth.orUndefined
