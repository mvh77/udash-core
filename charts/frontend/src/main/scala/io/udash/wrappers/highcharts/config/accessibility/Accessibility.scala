...
object Accessibility {
  ...
  /**
    ...
    */
  def apply(describeSingleSeries: Option[Boolean] = None,
            enabled: Option[Boolean] = None,
            keyboardNavigation: Option[AccessibilityKeyboardNavigation] = None,
            onTableAnchorClick: Option[() => Unit] = None,
            pointDateFormat: Option[String] = None,
            pointDateFormatter: Option[(api.Point) => String] = None,
            pointDescriptionFormatter: Option[(api.Point) => String] = None,
            pointDescriptionThreshold: Option[Double | Boolean] = None,
            screenReaderSectionFormatter: Option[(api.Chart) => String] = None,
            seriesDescriptionFormatter: Option[(api.Series) => String] = None): Accessibility = {
    val describeSingleSeriesOuter = describeSingleSeries.orUndefined
    val enabledOuter = enabled.orUndefined
    val keyboardNavigationOuter = keyboardNavigation.orUndefined
    val onTableAnchorClickOuter = onTableAnchorClick.map(js.Any.fromFunction0).orUndefined
    val pointDateFormatOuter = pointDateFormat.orUndefined
    val pointDateFormatterOuter = pointDateFormatter.map(js.Any.fromFunction1[api.Point, String]).orUndefined
    val pointDescriptionFormatterOuter = pointDescriptionFormatter.map(js.Any.fromFunction1[api.Point, String]).orUndefined
    val pointDescriptionThresholdOuter = pointDescriptionThreshold.orUndefined
    val screenReaderSectionFormatterOuter = screenReaderSectionFormatter.map(js.Any.fromFunction1[api.Chart, String]).orUndefined
    val seriesDescriptionFormatterOuter = seriesDescriptionFormatter.map(js.Any.fromFunction1[api.Series, String]).orUndefined
