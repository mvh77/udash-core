...
object Pane {
  ...
  /**
    ...
    */
  def apply(background: Option[Seq[PaneBackground]] = None,
            center: Option[Seq[Double | String]] = None,
            endAngle: Option[Double] = None,
            size: Option[Double | String] = None,
            startAngle: Option[Double] = None): Pane = {
    val backgroundOuter = background.map(_.toJSArray).orUndefined
    val centerOuter = center.map(_.toJSArray).orUndefined
